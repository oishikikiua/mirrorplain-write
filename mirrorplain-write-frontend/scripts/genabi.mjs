...
const CONTRACT_NAME = "MirrorPlainWrite";

// Path to fhevm-hardhat-template (relative from frontend root)
...
function readDeployment(chainName, chainId, contractName, optional) {
  ...
  if (!fs.existsSync(chainDeploymentDir) && chainId === 31337) {
    console.log(`📦 Attempting to auto-deploy ${contractName} on localhost...`);
    deployOnHardhatNode();
  }
  ...
  if (!fs.existsSync(contractFile)) {
    console.error(`${line}Contract file not found: ${contractFile}${line}`);
    if (!optional) {
      process.exit(1);
    }
    return undefined;
